...
// LVGL configuration for the WebAssembly item renderer.
//
// This is port/pc/lv_conf.h with exactly one value changed:
//
//   LV_USE_SDL                           0
//        There is no window. The module renders into an off-screen canvas and
//        hands the pixels to JavaScript; the display LvglHeadless.cpp creates
//        is a formality, because lv_obj_create() wants a screen and a screen
//        wants a display. Leaving SDL on would compile LVGL's own SDL driver,
//        which includes <SDL2/SDL.h> and expects a main loop and a window
//        neither of which exist here.
//
// Everything else is identical, and three of them are what make a browser
// build possible at all: LV_USE_OS is LV_OS_NONE and
// LV_DRAW_SW_DRAW_UNIT_CNT is 1, so there are no threads to want pthreads,
// SharedArrayBuffer and cross-origin isolation for; and LV_USE_STDLIB_MALLOC
// is LV_STDLIB_CLIB, which under emscripten is the wasm heap.
//
// Keep the two files in step. A divergence between them is a divergence
// between what the editor previews and what the panel draws.
//
// Below this header it is lvgl/lv_conf_template.h for 9.5.0, deliberately left
// in LVGL's own style -- doxygen comments, spaces and all -- so that an LVGL
// upgrade is a re-copy and a diff rather than an archaeology session. It is
// excluded from the kanardia-style skill for the same reason.
//
// What differs from that template, all of it things the board gets from
// Kconfig instead:
//
//   LV_USE_STDLIB_MALLOC/STRING/SPRINTF  LV_STDLIB_CLIB
//        A desktop has malloc. The built-in 64 kB pool would not hold the
//        400x400 ARGB8888 canvas the scenes draw into, never mind ThorVG's
//        own scratch.
//   LV_COLOR_DEPTH                       32
//        What SDL wants. The panel is RGB565 and the board is built that way;
//        the canvas itself is ARGB8888 in both builds, which is the part that
//        matters -- see "The canvas must stay ARGB8888" in CLAUDE.md.
//   LV_USE_VECTOR_GRAPHIC                1
//   LV_USE_THORVG_INTERNAL               1
//   LV_USE_MATRIX                        1
//   LV_USE_FLOAT                         1
//        The whole point: ThorVG through the lv_draw_vector_* API. The matrix
//        is what the descriptor's transform is made of and will not compile
//        without floats; lv_draw_vector.h will not compile without the matrix.
//   LV_USE_SNAPSHOT                      1
//        The console's screenshot command.
//   LV_USE_OBJ_PROPERTY                  1
//        lvgl_cpp's PropertySetters mixin uses lv_property_t unconditionally
//        on 9.5+, and that type only exists with this on.
//   LV_USE_SDL                           0
//        The one value that differs from port/pc/lv_conf.h. See above.
//
// Deliberately left alone:
//
//   LV_USE_LOG      0   Nothing here is worth a line per draw in the
//                       browser's console.
//   LV_USE_OS       LV_OS_NONE
//                       One thread, which in a browser is not a preference
//                       but the default: threads would mean pthreads,
//                       SharedArrayBuffer and cross-origin isolation on
//                       whatever server hosts the editor.
//   LV_DRAW_SW_DRAW_UNIT_CNT 1
//                       No ThorVG worker pool, for the same reason, and as on
//                       the board.
//   LV_USE_SNAPSHOT 1   Not used here -- the module reads the canvas's own
//                       draw buffer rather than snapshotting a screen -- but
//                       kept so the two configurations differ in one place
//                       only.
