...
//
// This is lvgl/lv_conf_template.h for 9.5.0 with eleven values changed, and is
// deliberately left in LVGL's own style below this header -- doxygen comments,
// spaces and all -- so that an LVGL upgrade is a re-copy and a diff rather than
// an archaeology session. It is excluded from the kanardia-style skill for the
// same reason.
//
// What differs from the template, all of it things the board gets from Kconfig
// instead:
//
...
//   LV_USE_SDL                           1
//        The display and pointer back end. LV_SDL_DIRECT_EXIT is already 1 in
//        the template, which is what makes closing the window end the
//        process -- the only tidy way out with the model and console threads
//        still running.
//
// Deliberately left alone:
//
//   LV_USE_LOG      0   LVGL's log goes to stdout, and the console's base64
//                       screenshots go out on the process's original stdout.
//                       PlatformSim.cpp separates the two, but there is
//                       nothing here worth the noise anyway.
//   LV_USE_OS       LV_OS_NONE
//                       LVGL is driven from one thread here (MainSim.cpp) and
//                       everything else takes platform::LockDisplay(), which
//                       is the shape the board has with the BSP's mutex.
//   LV_DRAW_SW_DRAW_UNIT_CNT 1
//                       No ThorVG worker pool, as on the board.
