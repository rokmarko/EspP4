...
// The instrument parameters, held in Common's own container.
//
...
public:
	...
	//
	// nod:  the network object data every parameter reads from. It has
	...
	//
	// sBlob is a single `parameter::fbs::ParamItem` flatbuffer -- what
	...
	//
	// Returns the id that changed, or can::Id::Invalid if the blob was not a
	//         parameter we hold.
