...
// The scale, drawn once, against any PainterLike back end.
//
...
public:
	...
	//
	// bands:  must already be in user units.
	// Returns false if the band range is empty, in which case nothing is drawn.
	static bool DrawArc(
		PainterLike auto&					P,
		const Arc2D&						arc,
	...
	// to place the needle.
	static std::pair<Arc2D, Arc2D> DrawArcIAS(
		PainterLike auto&					P,
		const Arc2D&						arc,
		const Markings&					markings,
...
protected:
	...
	static void
	DrawTriangle(PainterLike auto& P, const Vec2D& ptC, float fAngleRad, float fRadius, float fLength, bool bDot);

	static void DrawMinorDashes(
		PainterLike auto&	 P,
		const Arc2D&		 arc,
		const RangeF&		 range,
		::gui::ARGB			 col,
	...
	static void
	DrawLabels(PainterLike auto& P, const Arc2D& arc, const RangeF& range, const Markings& markings, float fOffset);

	// Prepares values for drawing a dash.
	static void
	DrawDash(PainterLike auto& P, float fX, const Arc2D& arc, const RangeF& range, float fDashLength, float fOffset);
	...
	static void
	DrawLabel(PainterLike auto& P, float fX, const Arc2D& arc, const RangeF& range, const char* pszText, float fOffset);

	static void
	DrawLabelAt(PainterLike auto& P, const Vec2D& ptC, float fAngleRad, float fR, const char* pszText, bool bInside);

	// Prepares values for drawing a band.
	static void
	DrawBand(PainterLike auto& P, const Arc2D& arc, const RangeF& rangeBand, const RangeF& rangeScale, float fR);
	...
	{
		return RangeF(range.GetLow() / fMultiples, range.GetHigh() / fMultiples);
	}
};
