...
// ThorVG vector graphics driven through the lvgl_cpp C++ binding.
//
...
parameter::Bands ParameterBands(can::Id eId)
{
	...
	return pModel != nullptr ? pModel->GetParameters().GetUserBands(eId) : parameter::Bands();
}
...
std::string UnitGlyph(unit::Key eKey)
{
	...
	if(ss.empty())
		ss = avio::format::ToString(eKey, UnitExtension::Signature);
	return ss;
}
...
//
// fUser:  value already in eKey, not in the function's system unit.
...
private:
	...
	// Triangular marker riding arc at relative position fRel.
	//
	...
	void DrawArcMarker(
		scale::tvg::Painter& P, const scale::Arc2D& arc, float fRel, float fLength, float fHalfWidth, uint32_t rgb
	);

	// Dial face disc, shared by the three instrument scenes.
	void DrawFace(scale::tvg::Painter& P);
	// One needle, built pointing up in local space and rotated to fAngleRad.
	void DrawNeedle(scale::tvg::Painter& P, float fAngleRad, float fLength, float fHalfWidth, uint32_t rgb);
	// Thin stick with a triangular tip -- the altimeter's ten-thousands hand.
	void
	DrawTipNeedle(scale::tvg::Painter& P, float fAngleRad, float fLength, float fHalfWidth, float fTip, uint32_t rgb);
...
void Scene::BuildScale()
{
	...
	m_arc = scale::Arc2D(scale::Vec2D(CTR, CTR), CTR - 26.0f, common::Rad(225.0f), common::Rad(-270.0f));
}
...
// A needle built pointing up in local space, then turned to fAngleRad.
//
...
void Scene::DrawNeedle(scale::tvg::Painter& P, float fAngleRad, float fLength, float fHalfWidth, uint32_t rgb)
{
	VectorDraw& dsc  = P.GetDraw();
	VectorPath& path = P.GetPath();
...
void Scene::DrawTipNeedle(
	scale::tvg::Painter& P, float fAngleRad, float fLength, float fHalfWidth, float fTip, uint32_t rgb
)
{
	VectorDraw& dsc  = P.GetDraw();
	VectorPath& path = P.GetPath();
...
void Scene::BuildIas()
{
	...
	m_iasArc = scale::Arc2D(scale::Vec2D(CTR, CTR), CTR - 52.0f, common::Rad(90.0f), common::Rad(-300.0f));
}
...
void Scene::DrawIas(scale::tvg::Painter& P)
{
	...
	const auto [aPre, aMain] = scale::tvg::Scale::DrawArcIAS(
		P, m_iasArc, m_iasMarkings, m_iasMarkingsIAS, m_iasBands, m_colors, m_iasStyle, m_iasStyleIAS
	);
	(void)aPre;
...
void Scene::BuildAltimeter()
{
	...
	m_altArc = scale::Arc2D(scale::Vec2D(CTR, CTR), CTR - 26.0f, common::Rad(90.0f), common::Rad(-360.0f));
}
...
void Scene::DrawAltimeter(scale::tvg::Painter& P)
{
	...
	DrawTipNeedle(P, m_altArc.GetAngle(Relative(100000.0f)), fR - 128.0f, 3.0f, 14.0f, 0xFFC145);
...
void Scene::BuildRpm()
{
	...
	m_engRpmArc = scale::Arc2D(scale::Vec2D(CTR - (R + GAP), CTR), R, common::Rad(-SPAN), common::Rad(2.0f * SPAN));
	...
	m_rotRpmArc =
		scale::Arc2D(scale::Vec2D(CTR + (R + GAP), CTR), R, common::Rad(180.0f + SPAN), common::Rad(-2.0f * SPAN));
}

void Scene::DrawArcMarker(
	scale::tvg::Painter& P, const scale::Arc2D& arc, float fRel, float fLength, float fHalfWidth, uint32_t rgb
)
{
	VectorDraw& dsc  = P.GetDraw();
...
void Scene::DrawRpm(scale::tvg::Painter& P)
{
	...
	scale::tvg::Scale::DrawArc(P, m_engRpmArc, m_engRpmMarkings, m_bands, m_colors, m_rpmStyle);
	scale::tvg::Scale::DrawArc(P, m_rotRpmArc, m_rotRpmMarkings, m_rotBands, m_colors, m_rpmStyle);

	// DrawArc() flushed its vector work before queueing labels, so the markers
	// added here land on top of everything.
	DrawArcMarker(P, m_engRpmArc, m_bands.GetRange().GetRelativeBounded(m_fRpm), 22.0f, 11.0f, ENGINE_RGB);
	DrawArcMarker(P, m_rotRpmArc, m_rotBands.GetRange().GetRelativeBounded(m_fRotorRpm), 22.0f, 11.0f, ROTOR_RGB);
...
void Scene::Tick()
{
	...
	if(const app::Model* pModel = app::GetModel()) {
		...
		m_fRpm		= m_bands.GetRange().GetLow() + m_bands.GetRange().GetSpan() * m_fValue / 100.0f;
	...
	switch(m_eMode) {
	case Mode::Gauge:
		m_valueLabel->set_text(Readout(m_fValue, Function::ThrottlePos, unit::Key::percent).c_str());
		break;
	case Mode::Scale: m_valueLabel->set_text(Readout(m_fRpm, Function::EngineRPM, unit::Key::RPM).c_str()); break;
	case Mode::Ias:	m_valueLabel->set_text(Readout(m_fIasKmh, Function::Airspeed, unit::Key::km_h).c_str()); break;
	case Mode::Altimeter:
		m_valueLabel->set_text(Readout(m_fAltFeet, Function::Altitude, unit::Key::feet).c_str());
		break;
	case Mode::Rpm:
...
bool Scene::Build()
{
	...
	const std::string ssGlyphs = UnitGlyph(unit::Key::C) + " " + UnitGlyph(unit::Key::F) + "  "
...
bool CaptureScreenshot(int step, PixelSink sink, void* ctx, int32_t* out_w, int32_t* out_h)
{
	...
	if(step == 1) {
		for(int32_t y = 0; y < src_h && ok; y++) {
			sink(snap->data + static_cast<size_t>(y) * stride, static_cast<size_t>(src_w) * 3, ctx);
		}
	}
	else {
