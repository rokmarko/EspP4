...
namespace { constexpr const char* TAG = "canproc"; } // namespace

// --------------------------------------------------------------------------

CanProcessor::CanProcessor(can::DirectNOD& nod, can::AbstractCanPort& port, uint32_t uSerial) :
	m_nod(nod),
	m_port(port),
	m_services(this),
...
void CanProcessor::Process(const can::Message& msg)
{
	...
	else if(eId == can::Id::APSRequest || eId == can::Id::APSResponse || eId == can::Id::APSPageCRC) {
...
int CanProcessor::GetAliveUnitCount() const
{
	...
	for(int i = 0; i < units.GetCapacity(); i++) {
		...
		if(pUnit != nullptr && pUnit->IsAlive())
			iCount++;
	}
	return iCount;
}
...
bool CanProcessor::GetDownloadData(
	uint32_t uiDataId, uint8_t byIndex, can::oldservice::DataType& eDataType, can::Register& rData
) const
{
	if(uiDataId != DDS_BUFFER)
		return false;
	// Message codes are 1-based, exactly as UnitInfoBase serves them.
	if(byIndex == 0 || byIndex > m_vSend.size())
		return false;
...
bool CanProcessor::PushBuffer(uint8_t byNodeB, can::oldservice::BufferDownloadCommand eCmd, common::SpanBLOB blob)
{
#if defined(USE_CAN_DDS_A)
	if(blob.empty())
		return false;
	...
	m_uPushApply =
		(static_cast<uint32_t>(blob.size()) << 16) | common::CRC16::Calc(blob.data(), static_cast<int>(blob.size()));
...
void CanProcessor::Pump()
{
	...
	for(int i = 0; i < BURST; ++i) {
		if(m_port.IsSpaceFor(2) == false)
			break;
		if(m_services.Update() == 0)
			break;
	}
	...
	if(m_bPushCommitPending && m_services.IsDownloadActive() == false) {
		...
		if(eState == can::oldservice::DDS_A::sSuccess) {
			m_services.ConfigureModule(
				m_uPushNode, MCS_APPLY_BUFFER_DATA, can::oldservice::dtULong, can::Register(m_uPushApply), m_uPushCmd
			);
		}
		else {
			APP_LOGE(
