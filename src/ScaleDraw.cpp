...
// Bodies of the scale drawing declared in ScaleDraw.h.
//
...
void Scale::DrawDash(
	PainterLike auto& P, float fX, const Arc2D& arc, const RangeF& range, float fDashLength, float fOffset
)
{
...
void Scale::DrawDash(PainterLike auto& P, const Vec2D& ptC, float fAngleRad, float fR1, float fR2)
{
	float fSA;
	float fCA;
...
void Scale::DrawLabel(
	PainterLike auto& P, float fX, const Arc2D& arc, const RangeF& range, const char* pszText, float fOffset
)
{
	const auto angle = arc.GetAngle(range.GetRelative(fX));
	DrawLabelAt(P, arc.GetCenter(), angle, arc.GetRadius() + fOffset, pszText, fOffset < 0);
}

// --------------------------------------------------------------------

void Scale::DrawLabelAt(
	PainterLike auto& P, const Vec2D& ptC, float fAngleRad, float fR, const char* pszText, bool bInside
)
{
...
void Scale::DrawLabels(
	PainterLike auto& P, const Arc2D& arc, const RangeF& range, const Markings& markings, float fOffset
)
{
...
void Scale::DrawBand(PainterLike auto& P, const Arc2D& arc, const RangeF& rangeBand, const RangeF& rangeScale, float fR)
{
...
void Scale::DrawBand(PainterLike auto& P, const Vec2D& ptC, float fR, float fStartDeg, float fSpanDeg)
{
...
)
{
	...
	DrawWhiteBand(
		P, a2, markingsIAS.GetWhiteRange(), bands.GetRange(), styleIAS.GetWhiteBand(), styleIAS.GetOffset().m_fWhiteBand
	);
...
{
	if(whiteRange.IsValidNonEmpty() == false)
		return;
...
)
{
	if(IsTriangle(mark.m_eShape)) {
		...
		DrawTriangle(
			...
			range,
			vStyle.m_fLength,
			styleIAS.GetOffset().m_fVTriangle,
...
void Scale::DrawTriangle(
	PainterLike auto& P, float fX, const Arc2D& arc, const RangeF& range, float fSideLength, float fOffset, bool bDot
)
{
	const auto angle = arc.GetAngle(range.GetRelative(fX));
...
void Scale::DrawTriangle(
	PainterLike auto& P, const Vec2D& ptC, float fAngleRad, float fRadius, float fLength, bool bDot
)
{
	float fSA;
	float fCA;
	...
	P.PushTransform(ptC.GetX() + fCA * fRadius, ptC.GetY() - fSA * fRadius, common::Deg(fAngleRad));
...
template bool Scale::DrawArc<PainterTvg>(
	PainterTvg&,
	const Arc2D&,
	const Markings&,
	const parameter::Bands&,
	const parameter::gui::Colors&,
	const style::Style&
);

template std::pair<Arc2D, Arc2D> Scale::DrawArcIAS<PainterTvg>(
	PainterTvg&,
	const Arc2D&,
	const Markings&,
	const MarkingsIAS&,
	...
	const parameter::gui::Colors&,
	const style::Style&,
	const style::StyleIAS&
);

#if defined(QT_CORE_LIB)

template bool Scale::DrawArc<PainterQt>(
	PainterQt&,
	const Arc2D&,
	const Markings&,
	const parameter::Bands&,
...
template std::pair<Arc2D, Arc2D> Scale::DrawArcIAS<PainterQt>(
	PainterQt&,
	const Arc2D&,
	const Markings&,
	const MarkingsIAS&,
