...
// The smallest concrete avio::ModelBase this board can carry.
//
...
public:
	...
	//
	// fSeconds:  seconds since boot, drives the synthetic engine run.
	...
	//
	// Returns false if the port is not in self-test, or a frame could not be
	//         posted.
...
protected:
	...
	const avio::gnss::ExtendedReceiverNMEA* GetExternalNMEAReceiverAlive() const override { return nullptr; }

	option::ModelBase* GetOptionsBase() override { return &m_options; }

	void SaveLastKnownCoordinate() override;

	void HandleNavigationChange(const avio::navigation::Action eAction, const uint32_t uActivateAxisCombo) override;

	void ActivateAutopilot(const uint32_t uAxisCombo, const avio::autopilot::Operation eOperation) override;
...
//
// Returns the can::Id that changed, or can::Id::Invalid if nothing was
