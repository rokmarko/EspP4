...
// The CANaerospace side of the bus: classify what arrives and file it.
//
...
public:
	...
	// lands.
	//
	// Returns false if a push is already running, or the offer could not be
	//         posted. Progress after that is reported by GetParameterPushCount()
	//         on the receiving side.
	bool PushBuffer(uint8_t byNodeB, can::oldservice::BufferDownloadCommand eCmd, common::SpanBLOB blob);
	...
	// their own thread.
	//
	// vOut:  filled with the flatbuffer, and left alone if none is ready.
	// Returns true if a parameter was taken; the slot is empty afterwards.
	...
	static can::uCUnitInfoContainer& Units() { return *can::uCUnitInfoContainer::GetInstance(); }
