...
uint32_t Settings::Save(Options& options, bool bDirtyOnly)
{
	...
	for(const option::Key eKey : options.GetKeys()) {
		...
		if(blob.empty()) {
			...
			ESP_LOGW(TAG, "option %u packed to nothing, not stored", static_cast<unsigned>(eKey));
			continue;
		}
...
bool Settings::LoadParameters(parameter::ParameterContainer& pc)
{
	...
	if(bAnyKnown == false) {
		ESP_LOGW(TAG, "parameter blob (%u B) did not unpack", static_cast<unsigned>(blob.size()));
		return false;
	}

	storage.ApplyTo(pc);
	ESP_LOGI(TAG, "parameters restored from a %u B blob", static_cast<unsigned>(blob.size()));
	return true;
}
...
bool Settings::SaveParameters(const parameter::ParameterContainer& pc)
{
	...
	ESP_LOGI(TAG, "%d parameters stored as a %u B blob", pc.GetCount(), static_cast<unsigned>(blob.size()));
	return true;
}
...
bool Settings::WriteBlob(const char* pcKey, common::SpanBLOB blob, bool bCommit)
{
	...
	if(err != ESP_OK) {
		ESP_LOGE(TAG, "nvs_set_blob('%s', %u B): %s", pcKey, static_cast<unsigned>(blob.size()), esp_err_to_name(err));
		return false;
	}
