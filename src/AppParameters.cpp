...
// the direction `Parameter::GetUserBands()` undoes.
	...
	)
	{
		...
		Parameter p(eId, eFunction, eSystemKey, 1, [&nod](can::Id eIdRead, uint8_t uIndex) {
			return nod.GetFloat(eIdRead, uIndex);
		});
...
Parameters::Parameters(const can::DirectNOD& nod)
{
	...
	Insert(MakeParameter(
		nod, can::Id::EngineRPM_1, Function::EngineRPM, unit::Key::RPM, rpm, unit::Key::RPM, "Engine RPM", "RPM", "RPM"
	));
	...
	Insert(MakeParameter(
		nod, can::Id::RotorRPM_1, Function::RotorRPM, unit::Key::RPM, rotor, unit::Key::RPM, "Rotor RPM", "Rotor", "ROT"
	));
